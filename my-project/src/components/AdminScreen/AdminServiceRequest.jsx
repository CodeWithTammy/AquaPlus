...
const filteredRequest = request
  ...
  .filter((r) =>
    r.full_name.toLowerCase().includes(search.toLowerCase()) || // case-insensitive match for name
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/requestservices/${id}`, {
        method: "DELETE",
      });
    ...
    <CardHeader floated={false} shadow={false} className="rounded-none">
      ...
      <div className="flex flex-col items-center justify-between gap-4 md:flex-row">
        ...
        <Tabs value={tab} className="w-full md:w-max" onChange={(value) => setTab(value)}>
          <TabsHeader className="bg-gray-100 p-1 mb-5 rounded-lg">
        ...
        {/* Search input field */}
        <div className="w-full mb-5 md:w-72">
          <Input
            className="pl-8"
    ...
    <CardBody className="overflow-scroll px-0">
      ...
      {selectedCustomerId && (
        <ViewDetailsCard
          ...
          type={"service"} // Specify type as service
        />
      )}
    </CardBody>
